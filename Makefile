...
run: clean ## Build, deploy and expose the application
	ko apply -B -f deploy
	kubectl expose deployment porthole --type=LoadBalancer --name=porthole --port 8081 --target-port 8081
...
print-ip: ## Print the IP address of the exposed service
	...
	@echo $$(kubectl get svc -n default porthole -o jsonpath='{.status.loadBalancer.ingress[0].ip}'):8081/
